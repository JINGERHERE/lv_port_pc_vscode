...
/* [移植改动] 说明：无需额外包含。
 * - Resource / GetImage / GetFont 由 MenuView.h -> ../Page.h -> ResourcePool.h 提供
 * - lv_get_indev() 由 ../Page.h -> lv_obj_ext_func.h 提供（移植时已改为 v9 的 lv_indev_get_type()）
 * - LVGL v9 用 lv_sdl_mousewheel_create() 创建的输入设备本身就是 LV_INDEV_TYPE_ENCODER，
 *   即“鼠标滚轮 = 旋钮旋转、鼠标中键 = 按下”，因此本页无需任何额外的编码器驱动代码。
 */
