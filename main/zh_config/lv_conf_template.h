...
#define LV_USE_ASSERT_STYLE         0   /**< 检查样式是否已正确初始化。（极快，推荐） */
#define LV_USE_ASSERT_MEM_INTEGRITY 0   /**< 在关键操作后检查 `lv_mem` 的完整性。（较慢） */
#define LV_USE_ASSERT_OBJ           0   /**< 检查对象的类型与存在性（如是否已被删除）。（较慢） */
...
#define LV_FONT_DEJAVU_16_PERSIAN_HEBREW    0  /**< 希伯来、阿拉伯、波斯字母及其全部字形 */
...
#define LV_USE_LOTTIE     0  /**< 依赖：lv_canvas、thorvg */
...
#if LV_USE_FS_STDIO
    #define LV_FS_STDIO_LETTER '\0'     /**< 为此驱动设置大写盘符标识字母（如 'A'）。 */
